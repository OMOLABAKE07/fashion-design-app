  ...
  getSyncStatus() {
    return {
      ...
      pendingItems: this.syncQueue.filter((item) => item.status === "pending")
        .length,
      retryItems: this.syncQueue.filter((item) => item.status === "retry")
        .length,
      failedItems: this.syncQueue.filter((item) => item.status === "failed")
        .length,
    };
  }
...
// Add missing methods to storageManager after initialization
storageManager.markAsSynced = function (type, id) {
  if (!this.data[type]) {
    return false;
  }
...
storageManager.getAllDesigns = function () {
  return this.data.designs || [];
};

storageManager.getAllMessages = function () {
  return this.data.messages || [];
};

storageManager.updateMessage = function (id, updates) {
  return this.updateItem("messages", id, updates);
};

storageManager.deleteMessage = function (id) {
  return this.deleteItem("messages", id);
};
...
export const syncUtils = {
  ...
  updateMessage(id, updates) {
    return storageManager.updateMessage(id, updates);
  },
  ...
  async updateCustomer(id, customerData) {
    ...
    const updatedCustomer = storageManager.updateItem(
      "customers",
      id,
      customerData
    );
  ...
  async saveMeasurement(measurementData) {
    const savedMeasurement = storageManager.addItem(
      "measurements",
      measurementData
    );
  ...
  async updateMeasurement(id, measurementData) {
    const updatedMeasurement = storageManager.updateItem(
      "measurements",
      id,
      measurementData
    );
  ...
  // Mark item as synced
  markAsSynced(type, id) {
    return storageManager.markAsSynced(type, id);
  },
