    ...
    <div v-if="showDetails" class="sync-details">
      <div class="details-content">
        ...
        <div class="detail-item">
          ...
          <span>{{
            lastSyncTime ? formatFullTime(lastSyncTime) : "Never"
          }}</span>
        </div>
  ...
  methods: {
    async initializeSync() {
      try {
        ...
        this.pendingChangesCount = Object.values(stats).reduce(
          (total, stat) => total + stat.unsynced,
          0
        );
      } catch (error) {
    ...
    async performSync() {
      ...
      try {
        ...
        this.$emit("sync-completed", {
          ...
          changesSynced:
            results && results.successful ? results.successful.length : 0,
        });
    ...
    async clearCache() {
      if (
        confirm(
          "Are you sure you want to clear the local cache? This will remove all unsynced changes."
        )
      ) {
        try {
