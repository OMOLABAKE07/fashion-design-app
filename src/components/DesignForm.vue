  ...
  <div class="design-form">
    <div class="form-header">
      ...
      <p class="form-description">
        Upload outfit photos and add detailed design notes including fabric
        type, style, and important dates.
      </p>
    </div>
      ...
      handler(newDesign) {
        if (newDesign) {
          this.formData = {
            ...
            designDate:
              newDesign.design_date || new Date().toISOString().split("T")[0],
  ...
  methods: {
    ...
    async loadCustomers() {
      try {
        ...
        this.customers = await response.data;
      } catch (error) {
        console.error(
          "Error loading customers from API, falling back to local storage:",
          error
        );
        ...
        try {
          ...
          console.error(
            "Error loading customers from local storage:",
            localError
          );
          this.customers = [];
        }
    ...
    async handleSubmit() {
      ...
      try {
        if (
          !this.formData.designName ||
          !this.formData.customerId ||
          !this.formData.designDate
        ) {
          Swal.fire({
            icon: "warning",
        ...
        try {
          ...
          window.dispatchEvent(
            new CustomEvent("design-saved", { detail: result.data || result })
          );
          ...
          try {
            ...
            let savedDesign;
            if (this.isEditing) {
              savedDesign = await syncUtils.updateDesign(
                this.design.id,
                designData
              );
            } else {
            ...
            window.dispatchEvent(
              new CustomEvent("design-saved", { detail: savedDesign })
            );
