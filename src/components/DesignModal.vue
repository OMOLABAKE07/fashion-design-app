        ...
        <div class="modal-body">
          <form @submit.prevent="handleSave" v-if="mode === 'edit'">
            ...
            <div class="form-section">
              ...
              <div class="photo-upload-area">
                ...
                >
                  ...
                  <div class="upload-content">
                    ...
                    <small
                      >Supported formats: JPG, PNG, GIF (Max 10MB each)</small
                    >
                  </div>
                </div>
              </div>
              ...
              <div
                v-if="editableDesign.photos && editableDesign.photos.length > 0"
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                ...
                <div class="form-group">
                  ...
                  <select
                    id="customer"
                    v-model="editableDesign.customerId"
                    required
                    class="form-select"
                  >
          ...
          <div v-else class="view-mode">
            ...
            <div
              ...
              v-if="
                (editableDesign.photos && editableDesign.photos.length > 0) ||
                editableDesign.photo_url
              "
            >
              <h4>Design Photos</h4>
              <div class="photos-gallery">
                ...
                <img
                  v-if="
                    editableDesign.photo_url &&
                    !isPhotoInArray(editableDesign.photo_url)
                  "
            ...
            <div class="detail-section">
              ...
              <div class="view-field">
                ...
                <span class="field-value">{{
                  editableDesign.designName || "-"
                }}</span>
              </div>
              <div class="view-field">
                <span class="field-label">Customer:</span>
                <span class="field-value">{{
                  getCustomerName(editableDesign.customerId) || "-"
                }}</span>
              </div>
              <div class="view-field">
                <span class="field-label">Design Date:</span>
                <span class="field-value">{{
                  formatDate(editableDesign.designDate) || "-"
                }}</span>
              </div>
              <div class="view-field">
            ...
            <div class="detail-section">
              ...
              <div class="view-field">
                ...
                <span class="field-value">{{
                  formatDate(editableDesign.firstFitting) || "-"
                }}</span>
              </div>
              <div class="view-field">
                <span class="field-label">Final Fitting Date:</span>
                <span class="field-value">{{
                  formatDate(editableDesign.finalFitting) || "-"
                }}</span>
              </div>
              <div class="view-field">
                <span class="field-label">Expected Completion Date:</span>
                <span class="field-value">{{
                  formatDate(editableDesign.completionDate) || "-"
                }}</span>
              </div>
              <div class="view-field">
                <span class="field-label">Delivery Date:</span>
                <span class="field-value">{{
                  formatDate(editableDesign.deliveryDate) || "-"
                }}</span>
              </div>
            </div>
            ...
            <div class="detail-section">
              ...
              <div class="view-field">
                ...
                <span class="field-value"
                  >${{ editableDesign.estimatedPrice || "0.00" }}</span
                >
              </div>
              <div class="view-field">
                <span class="field-label">Final Price:</span>
                <span class="field-value"
                  >${{ editableDesign.finalPrice || "0.00" }}</span
                >
              </div>
              <div class="view-field">
                <span class="field-label">Part Payment:</span>
                <span class="field-value"
                  >${{ editableDesign.partPayment || "0.00" }}</span
                >
              </div>
              <div class="view-field">
                <span class="field-label">Balance to Pay:</span>
                <span class="field-value"
                  >${{ editableDesign.balanceToPay || "0.00" }}</span
                >
              </div>
              <div class="view-field">
                <span class="field-label">Additional Notes:</span>
                <span class="field-value">{{
                  editableDesign.notes || "-"
                }}</span>
              </div>
            </div>
          </div>
...
import { customerAPI, designAPI } from "@/services/api.js";
...
export default {
  ...
  methods: {
    calculateBalance() {
      ...
      this.editableDesign.balanceToPay =
    ...
    isPhotoInArray(photoUrl) {
      ...
      return this.editableDesign.photos.some(
        (photo) =>
    ...
    setEditableDesign(design) {
      this.editableDesign = {
        ...
        specialInstructions:
          design.special_instructions || design.specialInstructions || "",
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (this.editableDesign.fabricType)
              formData.append("fabric_type", this.editableDesign.fabricType);
            if (this.editableDesign.color)
              formData.append("color", this.editableDesign.color);
            if (this.editableDesign.style)
              formData.append("style", this.editableDesign.style);
            if (this.editableDesign.occasion)
              formData.append("occasion", this.editableDesign.occasion);
            if (this.editableDesign.specialInstructions)
              formData.append(
                "special_instructions",
                this.editableDesign.specialInstructions
              );
            if (this.editableDesign.firstFitting)
              formData.append(
                "first_fitting",
                this.editableDesign.firstFitting
              );
            if (this.editableDesign.finalFitting)
              formData.append(
                "final_fitting",
                this.editableDesign.finalFitting
              );
            if (this.editableDesign.completionDate)
              formData.append(
                "completion_date",
                this.editableDesign.completionDate
              );
            if (this.editableDesign.deliveryDate)
              formData.append(
                "delivery_date",
                this.editableDesign.deliveryDate
              );
            if (this.editableDesign.estimatedPrice)
              formData.append(
                "estimated_price",
                this.editableDesign.estimatedPrice
              );
            if (this.editableDesign.finalPrice)
              formData.append("final_price", this.editableDesign.finalPrice);
            if (this.editableDesign.partPayment)
              formData.append("part_payment", this.editableDesign.partPayment);
            if (this.editableDesign.balanceToPay)
              formData.append(
                "balance_to_pay",
                this.editableDesign.balanceToPay
              );
            if (this.editableDesign.notes)
              formData.append("notes", this.editableDesign.notes);
            ...
            const result = await designAPI.update(
              this.editableDesign.id,
              formData
            );
      ...
      files.forEach((file) => {
        if (file.type.startsWith("image/")) {
          ...
          reader.onload = (e) => {
            ...
            const exists = this.editableDesign.photos.some(
