  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        let savedToDb = null
        try {
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}measurements`, {
            method: "POST",
    ...
    async saveEditedMeasurement(updatedMeasurement) {
      try {
        ...
        let savedToDb = null
        try {
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}measurements/${updatedMeasurement.id}`, {
            method: 'PUT',
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
