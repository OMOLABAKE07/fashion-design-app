  ...
  methods: {
      ...
      try {
        ...
        if (forceRefresh || this.measurementHistory.length === 0) {
          try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}measurements/customer/${this.customer.id}`)
    ...
    async saveEditedMeasurement(updatedMeasurement) {
      try {
        ...
        let savedToDb = null
        try {
          const response = await fetch(`${import.meta.env.VITE_API_URL}measurements/${updatedMeasurement.id}`, {
            method: 'PUT',
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
