  ...
  <div class="measurement-form">
    <div class="form-header">
      <div class="header-content">
        <h3>
          {{ isEditing ? "Update Measurements" : "Record New Measurements" }}
        </h3>
      ...
      <template v-for="category in [selectedCategory]" :key="category">
        ...
        <div class="measurement-section" v-if="category === 'female'">
          ...
          <div class="measurement-grid">
            ...
            <div class="measurement-group">
              <label for="shoulderToUpperBust"
                >Shoulder to Upper Bust (inches)</label
              >
              <input
                type="number"
            ...
            <div class="measurement-group">
              <label for="shoulderToUnderBust"
                >Shoulder to Under Bust (inches)</label
              >
              <input
                type="number"
            ...
            <div class="measurement-group">
              <label for="shoulderToNipplePoint"
                >Shoulder to Nipple Point (inches)</label
              >
              <input
                type="number"
            ...
            <div class="measurement-group">
              ...
              <input
                type="number"
                id="waistToHipLine"
                v-model="formData.waistToHipLine"
                step="0.25"
                min="0"
                class="measurement-input"
                placeholder="0.00"
            ...
            <div class="measurement-group">
              <label for="threeQuarterGownLength"
                >3/4 Gown Length (inches)</label
              >
              <input
                type="number"
    ...
    <div v-if="showHistory" class="measurement-history">
      ...
      <div v-if="measurementHistory.length === 0" class="no-history">
        <p>
          No measurements found{{ customer ? ` for ${customer.name}` : "" }}.
        </p>
      </div>
        ...
        >
          <div class="history-header">
            ...
            <div class="history-actions">
              ...
              <button @click="editMeasurement(measurement)" class="btn-small">
                Edit
              </button>
              <button
          ...
          <div class="measurement-summary">
            ...
            <span
              >Categories: {{ formatCategories(measurement.categories) }}</span
            >
            <span v-if="measurement.chest"
              >Chest: {{ measurement.chest }}"</span
            >
            <span v-if="measurement.waist"
              >Waist: {{ measurement.waist }}"</span
            >
...
import { customerAPI, measurementAPI } from "@/services/api";
...
export default {
  ...
  methods: {
    saveMeasurementHistory(updatedMeasurement) {
      const index = this.measurementHistory.findIndex(
    ...
    openDropdown() {
      ...
      this.filteredCustomers = this.customerSearchQuery.trim()
    ...
    async loadMeasurementHistory(forceRefresh = false) {
      try {
        ...
        if (forceRefresh || this.measurementHistory.length === 0) {
          try {
            const result = await measurementAPI.getByCustomerId(
              this.customer.id
            );
            ...
            measurements =
              syncUtils
                .getAllMeasurements()
                ?.filter((m) => m.customerId === this.customer.id) || [];
          }
        }
        ...
        this.measurementHistory = measurements.map((measurement) => {
          ...
          return {
            ...
            shoulderToUnderBust:
              measurementData.measurements?.shoulderToUnderBust || "",
            shoulderToUpperBust:
              measurementData.measurements?.shoulderToUpperBust || "",
            shoulderToNipplePoint:
              measurementData.measurements?.shoulderToNipplePoint || "",
            ...
            shortGownLength:
              measurementData.measurements?.shortGownLength || "",
            threeQuarterGownLength:
              measurementData.measurements?.threeQuarterGownLength || "",
      ...
      try {
        if (!this.customer || !this.customer.id) {
          Swal.fire({
            ...
            text: "Please select a customer before saving measurements",
          });
          return;
        }
        ...
        if (!this.selectedCategory) {
          Swal.fire({
            ...
            text: "Please select a measurement category",
          });
          return;
        }
        ...
        let savedToDb = null;
        try {
          savedToDb = await measurementAPI.create(measurementData);
        } catch (apiError) {
    ...
    async saveEditedMeasurement(updatedMeasurement) {
      try {
        let savedToDb = null;
        try {
          savedToDb = await measurementAPI.update(updatedMeasurement.id, {
            ...
            measurements: updatedMeasurement.measurements,
          });
        } catch (apiError) {
        ...
        const historyIndex = this.measurementHistory.findIndex(
          (m) => m.id === updatedMeasurement.id
        );
        if (historyIndex !== -1) {
          this.measurementHistory[historyIndex] = {
            ...
            nippleToNipple:
              updatedMeasurement.measurements.nippleToNipple || "",
            roundUpperBust:
              updatedMeasurement.measurements.roundUpperBust || "",
            roundUnderBust:
              updatedMeasurement.measurements.roundUnderBust || "",
            shoulderToUnderBust:
              updatedMeasurement.measurements.shoulderToUnderBust || "",
            shoulderToUpperBust:
              updatedMeasurement.measurements.shoulderToUpperBust || "",
            shoulderToNipplePoint:
              updatedMeasurement.measurements.shoulderToNipplePoint || "",
            halfLength: updatedMeasurement.measurements.halfLength || "",
            blouseLength: updatedMeasurement.measurements.blouseLength || "",
            longGownLength:
              updatedMeasurement.measurements.longGownLength || "",
            shortGownLength:
              updatedMeasurement.measurements.shortGownLength || "",
            threeQuarterGownLength:
              updatedMeasurement.measurements.threeQuarterGownLength || "",
            ...
            agbadaShoulder:
              updatedMeasurement.measurements.agbadaShoulder || "",
            ...
            bustUpperChest:
              updatedMeasurement.measurements.bustUpperChest || "",
